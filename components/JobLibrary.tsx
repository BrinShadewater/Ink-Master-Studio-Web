...
const JobThumbnail: React.FC<{ job: StudioJob }> = ({ job }) => {
  const [url, setUrl] = useState<string | null>(null);

  useEffect(() => {
    if (!job.sourceArtwork?.blob) {
      setUrl(null);
      return undefined;
    }
    const nextUrl = URL.createObjectURL(job.sourceArtwork.blob);
    setUrl(nextUrl);
    return () => URL.revokeObjectURL(nextUrl);
  }, [job.sourceArtwork?.blob]);

  if (!url) {
    return (
      <div className="flex h-16 w-16 flex-none items-center justify-center rounded-lg border border-slate-800 bg-slate-950 text-[10px] font-black uppercase tracking-wide text-slate-600">
        No art
      </div>
    );
  }

  return <img src={url} alt="" className="h-16 w-16 flex-none rounded-lg border border-slate-800 bg-slate-950 object-contain" />;
};
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="job-library-title">
      <div className="flex max-h-[min(760px,92dvh)] w-full max-w-3xl flex-col overflow-hidden rounded-2xl border border-slate-700 bg-slate-950 shadow-2xl shadow-black/60">
        <header className="flex items-center justify-between border-b border-slate-800 px-5 py-4">
          <div>
            <h2 id="job-library-title" className="text-lg font-black text-white">Saved designs</h2>
            <p className="mt-1 text-xs text-slate-400">Reopen, duplicate, export, or archive local designs.</p>
          </div>
        ...
        <div className="flex items-center gap-3 border-b border-slate-800 px-5 py-3">
          <button type="button" onClick={() => importInputRef.current?.click()} className="rounded-lg bg-indigo-600 px-4 py-2 text-xs font-bold text-white hover:bg-indigo-500">Import design archive</button>
          <input
            ref={importInputRef}
            type="file"
          ...
          />
          <span className="text-xs text-slate-500">{jobs.length} saved {jobs.length === 1 ? 'design' : 'designs'} on this device</span>
        </div>

        <div className="min-h-0 flex-1 overflow-y-auto p-3">
          {jobs.length === 0 ? (
            <div className="rounded-xl border border-dashed border-slate-700 px-6 py-14 text-center">
              <p className="text-sm font-bold text-slate-300">No saved designs yet</p>
              <p className="mt-2 text-xs text-slate-500">Drop artwork to create one, or import a design archive.</p>
            </div>
          ) : (
                  ...
                  <article key={job.id} className={`rounded-xl border p-4 ${isCurrent ? 'border-indigo-500/60 bg-indigo-500/10' : 'border-slate-800 bg-slate-900/60'}`}>
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        ...
                        <div className="flex min-w-0 items-center gap-3">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="mt-1 truncate text-xs text-slate-400">
                              {job.sourceArtwork?.name ? filenameToDesignName(job.sourceArtwork.name) : 'Artwork not attached'}
                            </p>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        ...
                        <button type="button" onClick={() => onExport(job)} className="rounded-md border border-slate-700 px-2.5 py-1.5 text-[11px] font-bold text-slate-300 hover:border-slate-500">Export archive</button>
