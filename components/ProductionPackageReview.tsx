...
export const ProductionPackageReview: React.FC<ProductionPackageReviewProps> = ({ review, currentStage, onNavigateToStage }) => {
  ...
  return (
    <section className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-4">
      ...
      {manifestIntegrity && (
        <div className="mt-3 rounded-lg border border-emerald-500/20 bg-slate-950/50 px-3 py-2">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <p className="text-[10px] font-bold uppercase tracking-widest text-emerald-300">Manifest-verified package</p>
              <p className="mt-1 text-[11px] leading-relaxed text-slate-300">{manifestIntegrity.note}</p>
            </div>
            <span className={`flex-none rounded-full border px-2 py-0.5 text-[9px] font-black ${readinessClass[manifestIntegrity.status]}`}>
              {readinessLabel[manifestIntegrity.status]}
            </span>
          </div>
        </div>
      )}
