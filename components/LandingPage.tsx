...
export const LandingPage = ({ onOpenEditor }: LandingPageProps) => <main className="min-h-dvh overflow-hidden bg-[#0b121a] text-neutral-100">
  ...
  <section className="relative z-10 mx-auto grid max-w-[1440px] items-center gap-10 px-5 py-12 md:px-8 lg:min-h-[690px] lg:grid-cols-[minmax(0,0.92fr)_minmax(580px,1.08fr)] lg:py-8">
    <div className="max-w-xl"><div className="inline-flex items-center gap-2 border border-[#496879] bg-[#121f2a] px-3 py-2 text-xs font-semibold uppercase tracking-[0.12em] text-[#86b8bb]">Canvas-first. Print-ready.</div><h1 className="mt-7 text-5xl font-black uppercase leading-[0.96] text-white md:text-6xl xl:text-7xl">Turn artwork into a <span className="text-[#6aa3a8]">print-ready</span> shirt design.</h1><p className="mt-6 max-w-lg text-base leading-7 text-[#b2c5c8] md:text-lg">Design with precision on an infinite canvas. Fine-tune the artwork, preview it on real garments, and export with confidence.</p><div className="mt-8 flex flex-wrap items-center gap-5"><button type="button" className="flex h-14 items-center gap-2 bg-[#315f6c] px-6 text-base font-bold uppercase tracking-wide text-white transition hover:bg-[#3d7781] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#9ac9ce]" onClick={onOpenEditor}><Sparkles aria-hidden="true" size={19} /> Start designing</button></div><div className="mt-11 hidden gap-4 border-t border-[#405b6b] pt-6 md:grid md:grid-cols-3">{steps.map(([Icon, title, detail]) => <div key={title}><Icon aria-hidden="true" className="text-[#7bb5b9]" size={25} /><p className="mt-2 text-sm font-bold uppercase text-white">{title}</p><p className="mt-1 text-xs leading-5 text-[#91aaae]">{detail}</p></div>)}</div></div>
    <ProductStage />
  </section>
  <section id="features" className="relative z-10 border-t border-[#36515f] bg-[#14232d] shadow-[0_-10px_28px_rgba(0,0,0,0.2)]"><div className="mx-auto grid max-w-[1440px] gap-6 px-5 py-8 md:grid-cols-[1.15fr_repeat(3,1fr)] md:px-8"><div><p className="text-xs font-bold uppercase tracking-[0.12em] text-[#9cc5c7]">Built for print precision</p><h2 className="mt-3 text-3xl font-black uppercase leading-none text-white">Every detail, dialed in.</h2></div>{[[Crosshair, 'Pixel-perfect control', 'Guides, alignment, and clean placement.'], [Download, 'Print-ready output', 'High-resolution PNGs for production.'], [Sparkles, 'Color you can trust', 'Finishing controls made for apparel.']].map(([Icon, title, detail]) => { const FeatureIcon = Icon as typeof Crosshair; return <div key={title as string} className="flex gap-3 border-l border-[#3a5664] pl-5"><FeatureIcon aria-hidden="true" className="shrink-0 text-[#90bec1]" size={25} /><div><h3 className="text-sm font-bold text-white">{title as string}</h3><p className="mt-1 text-sm leading-6 text-[#c1ced2]">{detail as string}</p></div></div>; })}</div></section>
</main>;
